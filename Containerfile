...
LABEL org.opencontainers.image.source="https://github.com/allthingslinux/astromorty" \
        org.opencontainers.image.description="Astromorty - The all in one discord bot for All Things Linux Community" \
        org.opencontainers.image.licenses="GPL-3.0" \
        org.opencontainers.image.authors="All Things Linux" \
        org.opencontainers.image.vendor="All Things Linux" \
        org.opencontainers.image.title="Astromorty" \
        org.opencontainers.image.documentation="https://github.com/allthingslinux/astromorty/blob/main/README.md"
...
COPY src/astromorty/database/migrations/ ./src/astromorty/database/migrations/
...
COPY src/ ./src/
RUN cp -a src/astromorty ./astromorty
...
RUN mkdir -p ./src/astromorty/plugins ./astromorty/plugins && \
        touch ./src/astromorty/plugins/.gitkeep ./astromorty/plugins/.gitkeep
...
COPY --from=build --chown=nonroot:nonroot /app/astromorty /app/astromorty
...
RUN ln -sf /app/.venv/bin/python /usr/local/bin/python && \
         ln -sf /app/.venv/bin/astromorty /usr/local/bin/astromorty
...
RUN set -eux; \
        ...
        /app/.venv/bin/python -m compileall -b -q /app/astromorty "${PYTHON_LIB_PATH:?}" 2>/dev/null || true

USER nonroot

HEALTHCHECK --interval=30s --timeout=10s --start-period=40s --retries=3 \
         CMD python -c "import astromorty.shared.config.settings; print('Health check passed')" || exit 1
