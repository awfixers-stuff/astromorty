...
The Astromorty project uses OpenCode's rules and commands system to provide AI-assisted development with project-specific patterns and workflows.
...
Rules are automatically applied by OpenCode based on file patterns, while commands are invoked manually with the `/` prefix in OpenCode chat.

## Structure

### Rules

Rules are organized by domain in `.opencode/rules/`:
...
- **`meta/`** - System documentation (OpenCode rules/commands specifications)

### Commands

Commands are organized by category in `.opencode/commands/`:
...
Rules are automatically applied by OpenCode:
...
- **Intelligent** - Rules with `description` are selected by OpenCode based on context

Commands are invoked manually:

1. Type `/` in OpenCode chat
