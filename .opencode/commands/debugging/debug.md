...
1. **Problem Analysis**
   ...
   - Examine Discord API error codes (reference `@discord-api-docs` for error code meanings)
   - Verify database state with `uv run db health`
2. **Discord Bot Debugging**
   - Enable debug mode: `uv run astromorty start --debug`
...
- Discord API Documentation: `@discord-api-docs` - Reference for Discord API error codes, gateway events, and debugging information
