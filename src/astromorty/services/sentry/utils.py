...
def capture_astromorty_exception(
    error: AstromortyError,
    *,
    command_name: str | None = None,
    user_id: str | None = None,
    guild_id: str | None = None,
) -> None:
    """Capture a AstromortyError with specialized context."""
    if not is_initialized():
        return
...
def convert_httpx_error(
    ...
    *,
    ...
    """
    Convert HTTPX error to AstromortyAPI exception and report to Sentry.
    ...
    if isinstance(error, httpx.HTTPStatusError):
        ...
        if status_code == 403:
            raise AstromortyAPIPermissionError(service_name=service_name) from error
    ...
    if isinstance(error, httpx.RequestError):
        ...
        raise AstromortyAPIConnectionError(
            service_name=service_name,
