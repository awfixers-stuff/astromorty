...
from astromorty.shared.constants import EMBED_COLORS
...
class HelpRenderer:
    ...
    async def create_main_embed(
        ...
        """
        if CONFIG.BOT_INFO.BOT_NAME != "Astromorty":
            embed = self.create_base_embed(
                "Hello! Welcome to the help command.",
                f"{CONFIG.BOT_INFO.BOT_NAME} is a self-hosted instance of Astromorty. The bot is written in Python using discord.py.\n\nIf you enjoy using {CONFIG.BOT_INFO.BOT_NAME}, consider contributing to the original project.",
            )
        else:
            embed = self.create_base_embed(
                "Hello! Welcome to the help command.",
                "Astromorty is an all-in-one bot by the All Things Linux Discord server. The bot is written in Python using discord.py, and we are actively seeking contributors.",
            )
    ...
    async def _add_bot_help_fields(self, embed: discord.Embed) -> None:
        ...
        bot_name_display = (
            "Astromorty"
            if CONFIG.BOT_INFO.BOT_NAME == "Astromorty"
            else f"{CONFIG.BOT_INFO.BOT_NAME} (Astromorty)"
        )
        owner_info = (
