...
from astromorty.services.sentry import capture_exception_safe
from astromorty.services.sentry.metrics import record_cog_metric
from astromorty.services.sentry.tracing import (
    capture_span_exception,
    set_span_attributes,
    span,
)
from astromorty.shared.config import CONFIG
from astromorty.shared.constants import (
    COG_PRIORITIES,
    MILLISECONDS_PER_SECOND,
    SLOW_COG_LOAD_THRESHOLD,
)
from astromorty.shared.exceptions import AstromortyCogLoadError, AstromortyConfigurationError
...
class CogLoader(commands.Cog):
    ...
    def _is_configuration_error(self, exception: Exception) -> bool:
        ...
        Walks the exception chain to detect AstromortyConfigurationError anywhere in
        ...
        -------
        bool
            True if the exception chain contains a AstromortyConfigurationError.
        """
        current_exception = exception

        while current_exception:
            if isinstance(current_exception, AstromortyConfigurationError):
                return True
            current_exception = (
    ...
    async def _load_single_cog(self, path: Path) -> None:
        ...
        try:
        ...
        except AstromortyConfigurationError as config_error:
            ...
            raise AstromortyCogLoadError(error_msg) from e
    ...
    async def load_cogs(self, path: Path) -> None:
        ...
        try:
            ...
            raise AstromortyCogLoadError(msg) from e
