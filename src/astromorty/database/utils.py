"""
Database Utilities for Astromorty Bot.
...
commands.Context, discord.Interaction, or Astromorty bot instances.
"""
...
def _resolve_bot(
    source: commands.Context[Astromorty] | discord.Interaction | Astromorty,
) -> Astromorty | None:
    ...
    if isinstance(source, discord.Interaction):
        # Check for Astromorty-specific attributes instead of isinstance to avoid circular import
        client = source.client
        if hasattr(client, "db_service") or hasattr(client, "db"):
            # Type cast is safe here because we've verified Astromorty-specific attributes
            return cast("Astromorty", client)
        return None
    return source
