"""Main Astromorty configuration using Pydantic Settings.
...
from astromorty.shared.constants import ENCODING_UTF8
...
class Config(BaseSettings):
    """Main Astromorty configuration using Pydantic Settings with multi-format support.
    ...
    def database_url(self) -> str:
        """Get database URL with proper host resolution and SSL support.
        ...
        Supports:
        - Supabase connection strings (automatically adds SSL if needed)
        - Standard PostgreSQL connection strings
        - Custom DATABASE_URL override
        """
        # Use explicit DATABASE_URL if provided
        if self.DATABASE_URL:
            url = self.DATABASE_URL
            # Ensure Supabase URLs use psycopg driver and SSL
