...
The Astromorty project uses Claude's rules and commands system to provide AI-assisted development with project-specific patterns and workflows.
...
Rules are automatically applied by Claude based on file patterns, while commands are invoked manually with the `/` prefix in Claude chat.

## Structure

### Rules

Rules are organized by domain in `.claude/rules/`:
...
- **`meta/`** - System documentation (Claude rules/commands specifications)

### Commands

Commands are organized by category in `.claude/commands/`:
...
Rules are automatically applied by Claude:
...
- **Intelligent** - Rules with `description` are selected by Claude based on context

Commands are invoked manually:

1. Type `/` in Claude chat
